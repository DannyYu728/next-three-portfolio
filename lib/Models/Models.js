...
export const Character = () => {
  ...
  const raycaster = useMemo(() => Raycaster(), [])
  const pointer = useMemo(() => Vector2(), [])
...
export const BasicModel = ({
  ...
  useEffect(() => {
    if (freeze && animations.length != 0 && actions[animation]) {
    ...
    !freeze && !hovered && actions.Animation?.stop()

    document.body.style.cursor = hovered ? 'pointer' : 'auto'
    return () => (document.body.style.cursor = 'auto')
  }, [hovered, mixer, actions, animation, animations.length, freeze])
